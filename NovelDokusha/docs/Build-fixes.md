...
## 11. Session 4 — Comprehensive Code/Build/Source Sweep (17 additional fixes)

> After the original 9 build-blockers (B1-B9) were fixed, a full codebase scan identified 28 remaining issues across data layer, scraper, translation, core/UI, and build/config. This section documents the 17 fixes applied in the latest sweep. The remaining 11 are intentional / back-compat / INFO-only — see `fixes.md` for the rationale.

### 11.1 T2 — Gemini API key/model captured at Scraper construction (lazy reads)

**Severity:** Medium — Settings changes had no effect until app restart.

**Files:** `scraper/.../Scraper.kt`, `scraper/.../sources/TimoTxtGemini.kt`, `scraper/.../sources/GeminiApiClient.kt`

**Root cause:** `Scraper` is `@Singleton` and originally did:
```kotlin
TimoTxtGemini(
    networkClient = networkClient,
    geminiApiKey = appPreferences.GEMINI_API_KEY.value,    // eager read
    geminiModel = appPreferences.GEMINI_MODEL.value,       // eager read
    geminiTemperatureProvider = { appPreferences.GEMINI_TEMPERATURE.value },
)
```
The first two values were captured at injection time. Updating them in Settings changed the SharedPreferences value but `Scraper`'s cached `String` was stale.

**Fix:** Changed both to providers (lambdas):
```kotlin
TimoTxtGemini(
    networkClient = networkClient,
...
```
`TimoTxtGemini` stores them as `() -> String` and forwards them to `GeminiApiClient`, which exposes `apiKey`/`model` as property getters that invoke the provider on each request. Settings changes now take effect on the next Gemini API call.

### 11.2 S2 — `TimoTxtGemini` used wrong OG image attribute

**Severity:** Low — cover images still worked via fallback.

**File:** `scraper/.../sources/TimoTxtGemini.kt` (2 occurrences: lines 235 and 390)

**Root cause:** Used `meta[name=og:image]` for cover image. The Open Graph standard uses `meta[property=og:image]`. The original `TimoTxt.kt` correctly uses `property=`.

**Fix:** Changed both `meta[name=og:image]` → `meta[property=og:image]`. Now the OG meta tag (the more reliable source) is used; the `.cover img[src]` fallback still exists for pages without OG metadata.

### 11.3 S3 — `TimoTxtTranslate` separator mismatch

**Severity:** Medium — batch title translation was silently falling through to a slower individual-translation fallback.

**File:** `scraper/.../sources/TimoTxtTranslate.kt:370-397`

**Root cause:** Joined batch titles with `" ||| "` but split the response on `" || | "` (different separator). The split always failed, falling through to a regex fallback `\\s*\\|{2,3}\\s*`.

**Fix:** Aligned both to `" ||| "` via a shared `separator` local val. Now the primary split succeeds and batch translation works as intended (much faster than translating titles individually).

### 11.4 S6 — `RoyalRoad.kt` catalogUrl mismatch

**Severity:** Low — `catalogUrl` is for display only.

**File:** `scraper/.../sources/RoyalRoad.kt:35`

**Root cause:** Declared `catalogUrl="https://www.royalroad.com/fictions/latest-updates?page=1"` but `getCatalogList` actually fetches `fictions/best-rated?page=...`.

**Fix:** Updated `catalogUrl` to `https://www.royalroad.com/fictions/best-rated?page=1`.

### 11.5 S7 — `MeioNovel.kt` selector typo

**Severity:** Medium — search-result cover extraction silently failed.

**File:** `scraper/.../sources/MeioNovel.kt:41`

**Root cause:** Selector `"tab-content-wrap .c-tabs-item .tab-thumb a"` was missing the leading `.` on `tab-content-wrap`. CSS treats a bare identifier as a tag selector, so this looked for a `<tab-content-wrap>` HTML element (which doesn't exist).

**Fix:** Added the leading `.`: `".tab-content-wrap .c-tabs-item .tab-thumb a"`.

### 11.6 S12 — `GeminiApiClient` used `Thread.sleep` in rate limiter

**Severity:** Medium — risk of thread-pool starvation under concurrency.

**File:** `scraper/.../sources/GeminiApiClient.kt` → `enforceRateLimit`

**Root cause:** Used `Thread.sleep(waitMs + 1000)` inside `synchronized(lock)`, blocking the calling coroutine's IO thread. With multiple concurrent translation batches, the IO dispatcher's threads could all be blocked sleeping.

**Fix:** Converted `enforceRateLimit()` to `suspend fun`. The wait-time computation still happens inside `synchronized(lock)` (for bookkeeping correctness), but the actual sleep now happens **outside** the lock via `delay(waitMs + 1000)`, which suspends the coroutine without blocking the thread.

### 11.7 S13 — `GeminiApiClient` used deprecated Gson API

**Severity:** Low — would break on Gson 3.x upgrade.

**File:** `scraper/.../sources/GeminiApiClient.kt` → `parseBatchResponse`

**Root cause:** Used `JsonParser.parseString(...).asJsonObject` — deprecated since Gson 3.x.

**Fix:** Replaced with `gson.fromJson(responseBody, JsonObject::class.java)` and added null-safe accessors on each `getAsJsonArray`/`getAsJsonObject` call (so malformed responses return `originalBatch` instead of throwing NPE).

### 11.8 S14 — `BookTextMapper` dead code

**Severity:** Low — dead code, no runtime impact.

**File:** `core/.../BookTextMapper.kt`

**Root cause:** `it.attr("src") ?: return null` was dead code — Jsoup's `Element.attr()` returns `""` (empty string) when the attribute is missing, never `null`.

**Fix:** Replaced with explicit blank check: `val src = it.attr("src"); if (src.isBlank()) return null`. Now empty/missing `src` attributes are actually rejected.

### 11.9 D4 — Backup restore reads entire zip into memory

**Severity:** Medium — OOM on devices with limited RAM.

**File:** `tooling/backup_restore/.../RestoreDataService.kt`

**Root cause:** `zipSequence` read the entire backup zip into memory via `associateWith { zipStream.readBytes() }`. A backup with many cover images could easily exceed the app's heap limit.

**Fix:** Replaced the materialized `associateWith` with a streaming loop over `ZipInputStream`:
```kotlin
    ...
    while (true) {
        ...
        when {
            entry.name == "database.sqlite3" -> {
                val dbBytes = zipStream.readBytes()  // only the DB entry in memory
                mergeToDatabase(dbBytes.inputStream())
            }
            entry.name.startsWith("books/") -> {
                val entryBytes = zipStream.readBytes()  // only this image in memory
...
```
Each entry is now read individually — peak memory is bounded by the largest single entry, not the total backup size.

### 11.10 D5 — Temp database file not deleted from filesystem

**Severity:** Medium — wasted disk space on every restore.

**File:** `tooling/backup_restore/.../RestoreDataService.kt`

**Root cause:** Temp database created with `name = "temp_database"` was never explicitly deleted. `clearDatabase()` only clears tables — the underlying SQLite file plus its journal/WAL files remained in the app's internal storage.

**Fix:** Added `context.deleteDatabase("temp_database")` after the merge loop. This closes the DB connection and deletes the file + journal + WAL.

### 11.11 D6 — Documented the `delay(1_000)` after cover image save

**Severity:** Low — 1-second delay, not breaking.

**File:** `data/.../LibraryBooksRepository.kt` → `saveImageAsCover`

**Root cause:** `delay(timeMillis = 1_000)` after writing the cover image — was an undocumented workaround.

**Fix:** Added an explanatory comment. The delay works around a MediaStore/indexing race on certain OEM ROMs (notably MIUI/EMUI) where the file isn't yet visible to the readback. Safe to remove once `MediaScannerConnection` is wired up properly.

### 11.12 BC1 — Removed `jcenter()` from settings and buildscript

**Severity:** Medium — JCenter sunset.

**Files:** `settings.gradle.kts`, `build.gradle.kts`

**Root cause:** `jcenter()` was declared in both `dependencyResolutionManagement` and `buildscript` blocks, with `noinspection JcenterRepositoryObsolete`. JCenter was deprecated in 2021 and shut down in 2022. No project dependency actually used it.

**Fix:** Removed `jcenter()` from both blocks.

### 11.13 BC2 — Renamed legacy configuration-cache property

**Severity:** Low — deprecation warnings.

**File:** `gradle.properties`

**Root cause:** Used the legacy `org.gradle.unsafe.configuration-cache=true` name. Gradle 8.2+ prefers the stable `org.gradle.configuration-cache=true` name.

**Fix:** Renamed to `org.gradle.configuration-cache=true` (and `-problems=warn`).

### 11.14 BC3 — Added `hilt-workmanager` to Hilt convention plugin

**Severity:** Low — future-proofing.

**File:** `build-logic/convention/src/main/kotlin/my/noveldoksuha/convention/plugin/Hilt.kt`

**Root cause:** `hilt-workmanager` (`androidx.hilt:hilt-work`) was only declared explicitly in `app/build.gradle.kts`, not in the convention plugin. Any library module that wanted to use `@HiltWorker` would need to declare it themselves.

**Fix:** Added `implementation(libs.findLibrary("hilt-workmanager").get())` to the `applyHilt()` convention plugin.

### 11.15 BC10 — Removed dormant `dependency-analysis` plugin declaration

**Severity:** Low — confusing for contributors.

**File:** `build.gradle.kts`

**Root cause:** `com.autonomousapps.dependency-analysis` (v1.31.0) was declared with `apply false` but never actually applied anywhere. `./gradlew buildHealth` did nothing.

**Fix:** Removed the dormant declaration. Re-add with `apply true` at the root project if you want it active.

### 11.16 C1 — `BaseActivity` now injects `AppPreferences` via Hilt

**Severity:** Low — duplicated instance machinery.

**File:** `coreui/.../BaseActivity.kt`

**Root cause:** Lazily created `AppPreferences(applicationContext)` instead of injecting the Hilt singleton. Both instances shared the same backing `SharedPreferences`, but the `OnSharedPreferenceChangeListener` flow machinery was duplicated.

**Fix:** Replaced `val appPreferences by lazy { AppPreferences(applicationContext) }` with `@Inject lateinit var appPreferences: AppPreferences`. `AppPreferences` is already `@Singleton @Inject constructor`, so Hilt provides the same instance used everywhere else.

### 11.17 C3 — Migrated deprecated `onBackPressed()` to `OnBackPressedDispatcher`

**Severity:** Low — works today, will break on future Android.

**Files:**
- `features/reader/.../ReaderActivity.kt` (had `override fun onBackPressed()`)
- 6 other activities using `::onBackPressed` as a callback reference:
  - `features/databaseExplorer/.../DatabaseBookInfoActivity.kt`
  - `features/databaseExplorer/.../DatabaseSearchActivity.kt`
  - `features/sourceExplorer/.../SourceCatalogActivity.kt`
  - `features/webview/.../WebViewActivity.kt`
  - `features/chaptersList/.../ChaptersActivity.kt`
  - `features/globalSourceSearch/.../GlobalSourceSearchActivity.kt`

**Root cause:** `Activity.onBackPressed()` is deprecated in API 33+.

**Fix:**
- `ReaderActivity` — replaced the override with an `OnBackPressedCallback` registered on `onBackPressedDispatcher` in `onCreate()`. The callback invokes `viewModel.onCloseManually()` (the original side-effect), disables itself, and redispatches to preserve the original "close activity" behavior.
- 6 other activities — replaced `::onBackPressed` with `onBackPressedDispatcher::onBackPressed`. Same behavior, no deprecation warning.

### 11.18 C4 — Moved hardcoded Gemini/Library strings to `strings.xml`

**Severity:** Low — non-English users saw English in Settings.

**Files:** `features/settings/.../sections/SettingsGemini.kt`, `features/settings/.../sections/LibraryAutoUpdate.kt`, `strings/src/main/res/values/strings.xml`

**Root cause:** 18 UI strings were hardcoded as Kotlin string literals:
- `"Gemini AI Translation"`, `"API Key"`, `"Get your free key at aistudio.google.com"`, `"Gemini API Key"`, `"Model"`, `"Select a preset or enter a custom model name"`, `"Flash (Best quality, 10 RPM/250 RPD)"`, `"Flash Lite (More quota, 30 RPM/1000 RPD)"`, `"Custom model..."`, `"Enter custom model name"`, `"Model name"`, `"Use preset instead"`, `"Temperature: ${String.format("%.2f", temperature)}"`, `"0.3 = consistent, 0.55 = balanced, 0.7 = creative"`, `"Status: Key configured. ..."` (×2)
- `"Library updates"`

**Fix:** Added 18 new string resources to `strings.xml`:
```xml
<string name="gemini_ai_translation">Gemini AI Translation</string>
<string name="gemini_api_key_label">API Key</string>
<string name="gemini_api_key_hint">Get your free key at aistudio.google.com</string>
<string name="gemini_api_key_field_label">Gemini API Key</string>
<string name="gemini_model_label">Model</string>
<string name="gemini_model_hint">Select a preset or enter a custom model name</string>
<string name="gemini_model_flash_best_quality">Flash (Best quality, 10 RPM/250 RPD)</string>
<string name="gemini_model_flash_lite_more_quota">Flash Lite (More quota, 30 RPM/1000 RPD)</string>
<string name="gemini_custom_model">Custom model…</string>
<string name="gemini_enter_custom_model_name">Enter custom model name</string>
<string name="gemini_model_name_label">Model name</string>
<string name="gemini_use_preset_instead">Use preset instead</string>
<string name="gemini_temperature_label">Temperature: %1$.2f</string>
<string name="gemini_temperature_hint">0.3 = consistent, 0.55 = balanced, 0.7 = creative</string>
<string name="gemini_status_key_configured">Status: Key configured. Use TimoTxt (Gemini) source to read novels with AI translation.</string>
<string name="gemini_status_no_api_key">Status: No API key. TimoTxt (Gemini) source requires a Gemini API key to translate.</string>
<string name="library_updates">Library updates</string>
```
Updated both Composables to use `stringResource(...)`.

### 11.19 C5 — Renamed misleadingly-named file

**Severity:** Low — contributor confusion.

**File:** `coreui/.../composableActions/onDoImportEPUB.kt`

**Root cause:** File was named `onDoImportEPUB.kt` but the function inside was `onDoAskForImage` — totally unrelated to EPUB import.

**Fix:** Renamed to `onDoAskForImage.kt`. (Pure rename, no content change.)

---

## 12. Building instructions (current state)

### 12.1 Local build (debug APK)

```bash
cd /path/to/NovelDokusha
./gradlew :app:assembleFullDebug      # full flavor (ML Kit translation)
./gradlew :app:assembleFossDebug      # foss flavor (no Google deps)
```

Output APKs land in `app/build/outputs/apk/full/debug/` and `app/build/outputs/apk/foss/debug/` respectively.

### 12.2 Local build (release APK)

```bash
# 1. Create a debug keystore at app/debug.keystore (for testing only):
...
# 2. Create custom.properties in the project root:
...
Output: `app/build/outputs/apk/full/release/app-full-release.apk` (~5 MB after R8 shrinking).

### 12.3 GitHub Actions

Two workflows are available:

- **`.github/workflows/build-apk.yml`** — runs on every push, builds the debug APK. No signing setup needed.
- **`.github/workflows/build-release-manual.yml`** — manual dispatch only. Builds the release APK (R8-shrunk, ~5 MB). Generates a debug keystore inline, so no secrets required.

### 12.4 Common build failures and their fixes

| Symptom | Cause | Fix |
|---|---|---|
| `MenuAnchorType` unresolved | Material3 1.2.1 doesn't have it (added in 1.3.0) | Use `.menuAnchor()` without args (see B4) |
| `no such column: chapterUrl` (Room migration) | Book table has no `chapterUrl` column | Use `url` instead (see D3) |
| `near "SET": syntax error` (Room migration) | Multiple `SET` keywords in one UPDATE | Use one `SET` keyword with comma-separated assignments (see D3) |
| `REPLACE called with 4 arguments` (Room migration) | SQLite's `REPLACE(X,Y,Z)` takes exactly 3 args | Nest `REPLACE(REPLACE(x, old1, new), old2, new)` (see D2) |
| Keystore not found at `app/storeFile.jsk` | `file()` in `app/build.gradle.kts` resolves relative to `app/`, not project root | Write keystore to `app/debug.keystore` and reference as `storeFile=debug.keystore` in `custom.properties` (see B5) |
| `unresolved reference: expectFirst` | No such extension function in codebase | Use `selectFirst(...)` and handle null (see B6) |
| `Scraper(...)` missing `appPreferences` arg | Constructor signature changed when Gemini source was added | Pass `appPreferences = ...` (see B7) |
| `org.gradle.unsafe.configuration-cache` deprecation warning | Legacy property name | Renamed to `org.gradle.configuration-cache` (see BC2) |

### 12.5 Verifying the build

After making changes, verify locally:
```bash
./gradlew :app:assembleFullDebug :app:assembleFossDebug --no-daemon
```

If that succeeds, the project compiles end-to-end. For a full release test, follow 12.2.

---

## See Also

- `fixes.md` — full bug tracker with severity ratings (37 issues, 26 fixed, 11 intentional/INFO)
- `DEPENDENCIES.md` — version catalog, convention plugins
- `ARCHITECTURE_OVERVIEW.md` — build flavors, types, signing
- `SECURITY.md` — signing, API key storage
