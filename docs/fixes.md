...
## Source Audit Pass (2026-06-30) — 12 additional fixes

A full HTTP probe of every registered source's `baseUrl` and `catalogUrl` revealed that 8 sources were pointing at dead/unreachable domains, 4 had moved to new TLDs, and the Cloudflare interceptor did not detect JS-challenge responses served with HTTP 200.

| # | Severity | Location | Status | Description |
|---|---|---|---|---|
| SA1 | 🟡 | `networking/.../UserAgentInterceptor.kt` | ✅ Fixed | UA was `Mozilla/5.0 (Windows NT 6.3; WOW64)` — a Windows 8.1-era string Cloudflare silently blocks. Updated to Chrome 120 + backfills 11 browser-like headers (`accept-language`, `sec-ch-ua`, `sec-fetch-*`, etc.). |
| SA2 | 🔴 | `networking/.../CloudfareVerificationInterceptor.kt` | ✅ Fixed | Interceptor only triggered on 403/503, missing the modern CF pattern of returning 200 + JS-challenge HTML body. Now also triggers on 429 and on 200-responses whose body contains `Just a moment...`, `cf-browser-verification`, `cf_chl_opt`, `cf-mitigated`, `challenge-platform`, etc. (peek 16 KB non-destructively). |
| SA3 | 🟡 | `scraper/.../sources/ReadLightNovel.kt` | ✅ Fixed | `readlightnovel.meme` expired → updated to `readlightnovel.org` (same site, same selectors). |
| SA4 | 🟡 | `scraper/.../sources/NovelBin.kt` | ✅ Fixed | `novelbin.me` expired → updated to `novelbin.net` (same site, same selectors). |
| SA5 | 🟡 | `scraper/.../sources/MTLNovel.kt` | ✅ Fixed | `www.mtlnovel.com` times out → updated to `mtlnovels.com` (canonical new domain). Search ajax URL also updated. |
| SA6 | 🟡 | `scraper/.../sources/BoxNovel.kt` | ✅ Fixed | `boxnovel.com` has broken SSL chain → updated to `boxnovel.org` (same Madara site). |
| SA7 | 🚨 | `scraper/.../sources/AT.kt` | ✅ Removed | `a-t.nu` DNS does not resolve. File deleted; source unregistered. |
| SA8 | 🚨 | `scraper/.../sources/MoreNovel.kt` | ✅ Removed | `morenovel.net` DNS gone; `morenovel.com` is a domain-sale placeholder. File deleted. |
| SA9 | 🚨 | `scraper/.../sources/Novelku.kt` | ✅ Removed | `novelku.id` DNS does not resolve. File deleted. |
| SA10 | 🚨 | `scraper/.../sources/MeioNovel.kt` | ✅ Removed | `meionovel.id` 301-redirects to `meionovels.com` which times out. File deleted. |
| SA11 | 🚨 | `scraper/.../sources/WbNovel.kt` | ✅ Removed | `wbnovel.com` taken over by unrelated Next.js app ("CoreSip"). File deleted. |
| SA12 | 🚨 | `scraper/.../sources/Wuxia.kt` | ✅ Removed | `wuxia.blog` DNS resolves but server doesn't respond; no working alternative with same structure. File deleted. |
| SA13 | 🚨 | `scraper/.../sources/LightNovelWorld.kt` | ✅ Removed | Site permanently shut down (returns `<title>Light Novel World Platform Shut Down</title>`). File deleted. |
| SA14 | 🚨 | `scraper/.../sources/Saikai.kt` | ✅ Removed | `saikaiscan.com.br` DNS gone; `api.saikai.com.br` also dead. No drop-in PT-BR replacement found. File deleted. |
| SA15 | 🟢 | `scraper/.../sources/AllNovel.kt` (new) | ✅ Added | New English source `https://allnovel.org/` to replace the shut-down LightNovelWorld. Plain HTML, no CF challenge. |
| SA16 | 🟢 | `scraper/.../Scraper.kt` | ✅ Fixed | Removed imports/registrations for the 8 deleted sources; added `AllNovel`. Final count: 21 catalog sources + 2 databases. |
| SA17 | 🟢 | `strings/.../strings-no-translatable.xml` | ✅ Fixed | Removed string resources for deleted sources; added `source_name_all_novel`. |
| SA18 | 🟢 | `app/.../androidTest/.../SourcesCatalogTest.kt` | ✅ Fixed | Removed UI tests for deleted sources (`source__saikai`, `source__wuxia`, `source__light_novel_world`); added `source__all_novel`. |

See `docs/SOURCES.md` for the full audit table, verification matrix, and per-source status.

## See Also

- `Build-fixes.md` — detailed log of all build fixes applied
- `SOURCES.md` — full source audit (2026-06-30): domain changes, removals, additions, CF improvements
