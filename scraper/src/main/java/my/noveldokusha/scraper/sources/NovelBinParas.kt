...
/**
 * NovelBin (Paras) — Alternative NovelBin scraper from the novelscraper project.
 *
 * Content structure: div#chr-content.chr-c with standard <p> tags.
 * This is an alternative to the built-in NovelBin source.
 * Both are interchangeable — if one fails to parse, the other may succeed.
 *
 * Adapted from the novelscraper project (Paras).
 */
class NovelBinParas(private val networkClient: NetworkClient) : SourceInterface.Catalog {
    override val id = "novelbin_paras"
    override val nameStrId = R.string.source_name_novelbin_paras
    ...
    override suspend fun getChapterText(doc: Document): String =
        withContext(Dispatchers.Default) {
            ...
            contentEl.select(".chr-end").remove()
            contentEl.select(".chr-start").remove()
            contentEl.select(".chr-jump").remove()
            contentEl.select(".chr-action").remove()
            contentEl.select(".chr-copyright").remove()
            ...
            text = text.replace(Regex("""(?i)novelbin\.me"""), "")
            text = text.replace(Regex("""(?i)novelbin\.com"""), "")
